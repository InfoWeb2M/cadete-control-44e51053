...
export async function fetchRelatorioMensal(mes?:number,ano?:number):Promise<RelatorioMensal>{
 const params=new URLSearchParams();if(mes)params.set('mes',String(mes));if(ano)params.set('ano',String(ano));
 return request(`/api/v1/performance/relatorio/mensal?${params}`);
}
